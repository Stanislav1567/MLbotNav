...
from mlbotnav.ml_alignment_calibration_params_audit import (
    audit_package_calibration_params_alignment,
    audit_registry_calibration_params_alignment,
)
from mlbotnav.ml_trade_dataset_contract import REQUIRED_COLUMNS


BASE_PARAMS = {"F051_break_buffer_pct": 0.06, "F051_confirm_bars": 1}


def _row(run_id: str, *, params: dict[str, object] | str | None = None) -> dict[str, str]:
    if params is None:
        params_text = json.dumps(BASE_PARAMS, ensure_ascii=False, sort_keys=True)
    ...
    return {
        ...
        "side": "SHORT",
        "calibration_params_json": params_text,
...
def _write_package(
    ...
    run_id: str,
    *,
    calibration_params: dict[str, object] | None = None,
    trade_params: dict[str, object] | str | None = None,
    oos_params: dict[str, object] | None = None,
        ...
        json.dumps(
            {
                ...
                "passport_id": "F051",
                "action_id": "F051_BOSDOWN_ALLOW",
                "calibration_params": calibration_params or BASE_PARAMS,
    ...
    with (package_dir / "trade_log.csv").open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writeheader()
        writer.writerow(_row(run_id, params=trade_params))
    if include_oos:
        oos_dir = package_dir / "source_reports"
        oos_dir.mkdir(parents=True, exist_ok=True)
        (oos_dir / "oos_report.json").write_text(
            json.dumps({"run_id": run_id, "calibration_params": oos_params or BASE_PARAMS}, ensure_ascii=False),
            encoding="utf-8",
        )
...
class MLAlignmentCalibrationParamsAuditTests(unittest.TestCase):
    def test_package_calibration_params_alignment_passes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            package = _write_package(root, "run_001")

            result = audit_package_calibration_params_alignment(project_root=root, package_path=package)
    ...
    def test_calibration_package_params_mismatch_fails(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            package = _write_package(root, "run_001", calibration_params={"F051_break_buffer_pct": 0.07})

            result = audit_package_calibration_params_alignment(project_root=root, package_path=package)

            self.assertEqual(result["status"], "FAIL")
            self.assertIn("calibration_params_mismatch", result["errors"])

    def test_trade_log_params_mismatch_fails(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            package = _write_package(root, "run_001", trade_params={"F051_break_buffer_pct": 0.07})
    ...
    def test_oos_params_mismatch_fails(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            package = _write_package(root, "run_001", oos_params={"F051_break_buffer_pct": 0.07})
    ...
    def test_invalid_trade_log_params_json_fails(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            package = _write_package(root, "run_001", trade_params="{bad-json")

            result = audit_package_calibration_params_alignment(project_root=root, package_path=package)

            self.assertEqual(result["status"], "FAIL")
            self.assertIn("invalid_trade_log_calibration_params_rows", result["errors"])
    ...
    def test_empty_registry_passes_as_no_approved_packages(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            result = audit_registry_calibration_params_alignment(project_root=root, registry_path=registry)
