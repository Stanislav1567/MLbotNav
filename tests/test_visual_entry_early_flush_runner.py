...
from mlbotnav.visual_entry_early_flush_runner import run_search


class VisualEntryEarlyFlushRunnerTests(unittest.TestCase):
    def test_runner_creates_early_flush_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["status"], "DEV_EARLY_FLUSH_REVERSAL_DIAGNOSTIC_NO_ML")
            self.assertTrue((tmp_path / "visual_entry_early_flush_reversal_20260512_DEV.json").exists())
            self.assertTrue((tmp_path / "visual_entry_early_flush_reversal_20260512_DEV_RU.md").exists())
