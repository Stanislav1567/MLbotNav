...
def _write_minimal_package(project_root: Path, run_id: str) -> Path:
...
class MLApprovalRegistryValidatorTests(unittest.TestCase):
    def test_empty_registry_passes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            result = validate_approval_registry(project_root=root, registry_path=registry)

            self.assertEqual(result["status"], "PASS")
            self.assertEqual(result["approved_count"], 0)
            self.assertEqual(result["failures"], [])

    def test_valid_approved_package_passes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            run_id = "oos_SOLUSDT_1m_test_short_only"
            package_dir = _write_minimal_package(root, run_id)
            registry = root / "configs" / "ml_approved_calibration_packages.yaml"
            _write_registry(
                registry,
                [
    ...
    def test_forbidden_status_fails(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            run_id = "oos_SOLUSDT_1m_test_short_only"
            package_dir = _write_minimal_package(root, run_id)
            ...
            self.assertIn("forbidden_status:NO_GO", result["failures"][0]["errors"])
