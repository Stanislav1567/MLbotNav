...
from mlbotnav.visual_entry_significant_low_selector_runner import run_search
...
def _row(start: datetime, idx: int, *, setup: bool = False) -> dict:
    return {
        ...
        "open": 100.0,
        "high": 100.20,
        "low": 99.70 if setup else 99.95,
        "close": 100.10 if setup else 100.0,
        "volume": 1000.0,
        "range_pos_60": 0.05 if setup else 0.90,
        "low_range_pos_60": 0.02 if setup else 0.90,
        "local_low_5": setup,
        "local_low_10": setup,
        "local_low_20": setup,
...
class VisualEntrySignificantLowSelectorRunnerTests(unittest.TestCase):
    def test_selector_uses_closed_signal_and_next_open_without_ml(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch(
                    "mlbotnav.visual_entry_significant_low_selector_runner._prepare_rows",
                    return_value=({}, rows),
                ),
                patch(
                    "mlbotnav.visual_entry_significant_low_selector_runner._enrich_quality_rows",
                    return_value=None,
                ),
                patch(
                    "mlbotnav.visual_entry_significant_low_selector_runner.render_family_candidate_overlay",
                    return_value=tmp_path / "overlay.png",
                ),
            ):
                payload = run_search(manual_entries, tmp_path, render_top=1)

            self.assertEqual(payload["status"], "DEV_SIGNIFICANT_LOW_SELECTOR_V1_ENTRY_ONLY_NO_ML")
            ...
            self.assertEqual(signal["signal_time_utc"], "2026-05-14T01:20:00Z")
            self.assertEqual(signal["entry_time_utc"], "2026-05-14T01:21:00Z")
