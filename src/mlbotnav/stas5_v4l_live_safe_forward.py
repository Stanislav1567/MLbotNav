...
from mlbotnav.stas5_v4l_live_safe_dataset import _add_live_safe_group_features, v4l_forbidden_feature_matches
from mlbotnav.stas5_v4l_live_safe_train import (
    DEFAULT_V4L_MODEL_DIR,
    DEFAULT_V4L_MODEL_MANIFEST_PATH,
    DEFAULT_V4L_MODEL_PATH,
    apply_v4l_live_decision_policy,
)
from mlbotnav.visual_entry_low_anchor_suggester import _load_ohlcv, _source_csv


STATUS = "STAS5_V4L_LIVE_SAFE_FORWARD_REVIEW_READY_BLIND_NO_FUTURE_NO_TP_NO_API_NO_STAS3"
SYMBOL = "SOLUSDT"
DEFAULT_START_DAY = "2026-05-26"
DEFAULT_END_DAY = "2026-05-30"
DEFAULT_V4L_FORWARD_ROOT = STAS5_ARTIFACTS_DIR / "v4l" / "forward"
DEFAULT_V4L_FORWARD_RUNS_DIR = DEFAULT_V4L_FORWARD_ROOT / "runs"
DEFAULT_V4L_FORWARD_SOURCE_DIR = STAS5_ARTIFACTS_DIR / "v4l" / "forward_source"
...
def _latest_model_manifest() -> Path:
    pointer = DEFAULT_V4L_MODEL_DIR / "STAS5_V4L_LATEST_MODEL_RUN.json"
    if pointer.exists():
    ...
    manifests = sorted(
        DEFAULT_V4L_MODEL_DIR.glob("runs/*/stas5_v4l_live_safe_group_ranker_20260501_20260525_v0.manifest.json"),
        key=lambda item: item.stat().st_mtime,
    )
    if manifests:
        return manifests[-1]
    if DEFAULT_V4L_MODEL_MANIFEST_PATH.exists():
        return DEFAULT_V4L_MODEL_MANIFEST_PATH
    raise FileNotFoundError("no STAS5 V4L model manifest found")
...
def _auto_group_forward_candidates_live_safe(
    rows: pd.DataFrame,
    *,
    max_gap_min: int = 75,
    ...
    for day, day_rows in out.sort_values(["day", "entry_time_utc", "record_id"]).groupby("day", sort=True):
        ...
        for idx, row in day_rows.iterrows():
            ...
            if new_group:
                group_idx += 1
                group_start = ts
            group_ids[idx] = f"G{compact_day(str(day))}_LIVE_{group_idx:03d}"
            last_ts = ts
    ...
    out["primary_reason_code"] = "FORWARD_LIVE_SAFE_AUTO_GROUP_REVIEW_ONLY"
    out["secondary_reason_codes"] = ""
    out["label_status"] = "FORWARD_REVIEW_ONLY"
    out["source_review_file"] = "AUTO_FORWARD_LIVE_SAFE_GROUPS"
    out["notes"] = "auto group for blind V4L live-safe forward ranking"
    return _add_live_safe_group_features(out)


def predict_v4l_forward(
    *,
    ...
    if forbidden:
        raise ValueError(f"forbidden V4L forward feature columns: {forbidden}")

    grouped = _auto_group_forward_candidates_live_safe(snapshot)
    ...
    out = grouped.copy()
    out["V4L_LIVE_SAFE_SCORE"] = scores
    thresholds = model_package.get("thresholds") or model_manifest.get("thresholds") or {"enter": 0.50, "unsure": 0.35}
    decision_policy = model_package.get("decision_policy") or model_manifest.get("decision_policy") or {}
    out = apply_v4l_live_decision_policy(
        out,
        score_column="V4L_LIVE_SAFE_SCORE",
        enter_threshold=float(thresholds["enter"]),
        unsure_threshold=float(thresholds["unsure"]),
        require_lowest_so_far=bool(decision_policy.get("require_lowest_so_far", True)),
    )
    out["V4L_DECISION"] = out["V4L_LIVE_DECISION"]
    out["V4L_GROUP_RANK_SCORE"] = out["V4L_LIVE_SAFE_SCORE"]
    out["V4_DECISION"] = out["V4L_LIVE_DECISION"]
    out["V4_GROUP_RANK_SCORE"] = out["V4L_LIVE_SAFE_SCORE"]
    out["ML_KEEP_SCORE_V2"] = out["V4L_LIVE_SAFE_SCORE"]
    out["ML_DECISION_V2"] = out["V4L_LIVE_DECISION"]
    out["ML_KEEP_SCORE"] = out["V4L_LIVE_SAFE_SCORE"]
    out["ML_DECISION"] = out["V4L_LIVE_DECISION"]
    out["model_id"] = model_package.get("model_id") or model_manifest.get("model_id") or "STAS5_V4L_LIVE_SAFE_GROUP_RANKER_V0"
    ...
    out["forward_policy"] = "live_safe_prefix_forward_no_future_no_day_end_top_n_no_threshold_tuning"
    return out


def _day_summary(rows: pd.DataFrame) -> dict[str, Any]:
    counts = Counter(rows["V4L_DECISION"].astype(str))
    top_entries = (
        rows[rows["V4L_DECISION"].astype(str) == "ENTER"]
        .sort_values(["entry_time_utc", "record_id"])[
            ["candidate_id", "group_id", "entry_time_utc", "entry_price_5bps", "V4L_LIVE_SAFE_SCORE"]
        ]
        .to_dict("records")
    )
    return {
...
def run_v4l_live_safe_forward_review(
    *,
    ...
    model_manifest_path: Path = DEFAULT_V4L_MODEL_MANIFEST_PATH,
    train_snapshot_manifest_path: Path = DEFAULT_V2_FEATURE_SNAPSHOT_MANIFEST_PATH,
    stas2_run_dir: Path | None = None,
    out_dir: Path = DEFAULT_V4L_FORWARD_ROOT,
    forward_source_dir: Path = DEFAULT_V4L_FORWARD_SOURCE_DIR,
    ...
    combo_path = out_dir / f"stas5_v4l_combo_features_{compact_day(start_day)}_{compact_day(end_day)}_forward.csv"
    ...
    if snapshot_manifest["status"] != "PASS":
        raise ValueError(f"V4L forward snapshot failed: {snapshot_manifest.get('checks')}")
    predictions = predict_v4l_forward(snapshot=snapshot, model_package=model_package, model_manifest=model_manifest)

    all_predictions_path = out_dir / f"STAS5_V4L_LIVE_SAFE_FORWARD_ALL_PREDICTIONS_{compact_day(start_day)}_{compact_day(end_day)}.csv"
    ...
    for day in iter_days(start_day, end_day):
        ...
        csv_path = day_dir / f"STAS5_V4L_LIVE_SAFE_FORWARD_ENTRIES_{compact_day(day)}.csv"
        png_path = day_dir / f"STAS5_V4L_LIVE_SAFE_FORWARD_REVIEW_{compact_day(day)}.png"
    ...
    manifest = {
        ...
        "decision_policy": model_package.get("decision_policy") or model_manifest.get("decision_policy"),
        ...
        "decision_counts_total": dict(Counter(predictions["V4L_DECISION"].astype(str))),
        "guardrails": [
            "forward_days_not_used_for_training",
            "forward_days_not_used_for_threshold_tuning",
            "auto_group_assignment_uses_only_previous_candidate_gap_and_group_age",
            "features_are_prefix_only_so_far_values",
            "no_day_end_top_n_selection",
            "past_decisions_are_not_rewritten_after_later_candidates",
            "postfact_columns_are_added_only_after_decision_for_visual_audit",
            "no_tp_stas3_api",
        ],
    }
    manifest_path = out_dir / "STAS5_V4L_LIVE_SAFE_FORWARD_REVIEW_MANIFEST.json"
    write_json(manifest_path, manifest)
    write_json(
        DEFAULT_V4L_FORWARD_ROOT / "STAS5_V4L_LATEST_FORWARD_RUN.json",
        {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run STAS5 V4L live-safe blind forward review.")
    ...
    parser.add_argument("--run-root", default=str(DEFAULT_V4L_FORWARD_RUNS_DIR))
    ...
    parser.add_argument("--forward-source-dir", default=str(DEFAULT_V4L_FORWARD_SOURCE_DIR))
    ...
    run_id = args.run_id or f"stas5_v4l_forward_{compact_day(args.start_day)}_{compact_day(args.end_day)}_{run_stamp()}"
    out_dir = Path(args.out_dir) if args.out_dir else Path(args.run_root) / run_id
    manifest = run_v4l_live_safe_forward_review(
