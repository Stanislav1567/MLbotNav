...
def _run_core_table_window_readiness(project_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    rp = write_data_window_readiness_report(
        project_root,
        payload=payload,
        action_name="daily_long_short_cycle_table_canon_core",
    )
    return {
...
def _prepare_table_canon(project_root: Path, args: argparse.Namespace, run_dir: Path) -> dict[str, Any]:
    if str(args.run_dir).strip() == "reports/table_canon_current":
        return _run(
            project_root,
            [
                sys.executable,
    ...
    parsed = prep.get("parsed_json") or {}
    source_run_dir = str(parsed.get("run_dir", "")).strip()
    if prep["returncode"] != 0 or not source_run_dir:
        return prep
    ...
    if not src.exists():
        prep["returncode"] = 1
        prep["stderr_tail"] = f"{prep.get('stderr_tail', '')}\nsource_run_dir_not_found={src}"
        return prep
    if run_dir.exists():
...
def _build_adaptive_cmd(args: argparse.Namespace, mode: str) -> list[str]:
    long_grid = str(args.p_long_grid if mode == "long_only" else args.p_long_grid_short_mode)
    short_grid = str(args.p_short_grid if mode == "short_only" else args.p_short_grid_long_mode)
    window_policy = "fixed_1d"
    if str(args.train_start) != str(args.train_end) or str(args.test_day) != str(args.test_end_day):
        window_policy = "multiday"
    cmd = [
        sys.executable,
        "-m",
    ...
    if not bool(getattr(args, "disable_hypothesis_profile", False)):
        cmd.append("--use-hypothesis-profile")
    if bool(getattr(args, "disable_hypothesis_profile", False)):
        cmd.append("--disable-hypothesis-profile")
    if bool(getattr(args, "disable_backlog_active_append", False)):
        cmd.append("--disable-backlog-active-append")
    return cmd


def main() -> int:
    parser = argparse.ArgumentParser(
        description="P5.3.7 daily cycle: run long/short adaptive, then 5+ table convergence audit and gate."
    )
    ...
    parser.add_argument("--modes", default="long_only,short_only", help="CSV subset of long_only,short_only")
    ...
    parser.add_argument("--disable-hypothesis-profile", action="store_true")
    parser.add_argument("--disable-backlog-active-append", action="store_true")
    ...
    out = qa_dir / f"daily_long_short_cycle_{_utc_tag()}.json"

    modes = []
    for m in str(args.modes).split(","):
        mm = m.strip().lower()
        if mm in {"long_only", "short_only"} and mm not in modes:
            modes.append(mm)
    if not modes:
    ...
    # Preflight must run before any heavy/canonical table steps.
    preflight_raw = _run(
        project_root,
        [
            sys.executable,
    ...
    preflight_raw["task"] = "preflight_window_raw_once"
    steps.append(preflight_raw)
    if preflight_raw["returncode"] != 0:
        failed = True

    if not failed:
        preflight_core = _run_core_table_window_readiness(project_root, args)
        preflight_core["task"] = "data_window_readiness_core_for_table_canon"
        steps.append(preflight_core)
        if preflight_core["returncode"] != 0:
            failed = True

    # Prepare canonical tables once per window (symbol/timeframe/test-day),
    # then reuse the same run_dir for long/short mode steps.
    if not failed:
        prep = _prepare_table_canon(project_root, args, run_dir)
        prep["task"] = "table_canon_pack_once"
        prep["applied_modes"] = modes
        steps.append(prep)
    ...
    for mode in modes:
        ...
        adaptive = _run(project_root, _build_adaptive_cmd(args, mode))
        adaptive["task"] = f"adaptive_auto_train_{mode}"
        steps.append(adaptive)
        if adaptive["returncode"] != 0:
            failed = True
            break
        parsed = adaptive.get("parsed_json") or {}
        summary_path_raw = str(parsed.get("summary_path", "") or "").strip()
        if not summary_path_raw:
            failed = True
            steps.append({"task": f"resolve_summary_{mode}", "error": "summary_path_missing"})
            break
        ...
        if not summary_path.exists():
            failed = True
            steps.append({"task": f"resolve_summary_{mode}", "error": "summary_path_not_found", "summary_path": str(summary_path)})
            break

        summary = _load_json(summary_path)
        oos_report = str((summary.get("best_oos") or {}).get("oos_report", "") or "").strip()
        if not oos_report:
            failed = True
            steps.append({"task": f"resolve_oos_{mode}", "error": "oos_report_missing_in_summary", "summary_path": str(summary_path)})
            break
        ...
        conv["task"] = f"table_convergence_5plus_{mode}"
        conv["summary_path"] = str(summary_path)
        conv["oos_report"] = oos_report
        steps.append(conv)
        if conv["returncode"] != 0:
            failed = True
            break

        if bool(args.disable_hypothesis_profile):
            steps.append(
                {
                    "task": f"hypothesis_coverage_audit_{mode}",
                    "returncode": 0,
                    "status": "SKIPPED_BY_POLICY",
                    "reason": "single_hypothesis_mode:disable_hypothesis_profile=true",
            ...
            hypo["task"] = f"hypothesis_coverage_audit_{mode}"
            steps.append(hypo)
            if hypo["returncode"] != 0:
                failed = True
                break
