...
def audit_smoke_window_manifest(*, project_root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if str(manifest.get("wbs_item") or "").strip() != "7.1":
        errors.append("invalid_wbs_item:not_7.1")
    if str(manifest.get("manifest_status") or "").strip() != "SMOKE_WINDOW_SELECTED":
    ...
    if all(parsed.values()):
        selected_dates = _date_range(parsed["train_start"], parsed["train_end"]) + _date_range(parsed["test_start"], parsed["test_end"])  # type: ignore[arg-type]
        forbidden_dates = {str(v).strip() for v in manifest.get("forbidden_dates", []) if str(v).strip()}
        used_forbidden = sorted(set(selected_dates).intersection(forbidden_dates))
        for day in used_forbidden:
            errors.append(f"forbidden_date:{day}")

    boundaries = manifest.get("stage_7_boundaries")
    boundaries = boundaries if isinstance(boundaries, dict) else {}
    for key in ("package_created", "package_approved_for_ml", "registry_mutated", "ml_training_started", "ml_ingest_started"):
    ...
    return {
        ...
        "reason": "OK" if not errors else "SMOKE_WINDOW_MANIFEST_FAIL",
        "manifest_path": _project_relative(root, resolved_manifest),
        ...
        "errors": errors,
        "boundary": {
            "package_created": bool(boundaries.get("package_created")),
            "package_approved_for_ml": bool(boundaries.get("package_approved_for_ml")),
            "registry_mutated": bool(boundaries.get("registry_mutated")),
            "ml_training_started": bool(boundaries.get("ml_training_started")),
            "ml_ingest_started": bool(boundaries.get("ml_ingest_started")),
        },
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit Stage 7.1 clean smoke window manifest.")
    parser.add_argument("--manifest-path", default="configs/ml_smoke_run_manifest.yaml")
    parser.add_argument("--out-dir", default="reports/qa_gate")
    args = parser.parse_args(argv)

    project_root = Path(__file__).resolve().parents[2]
    result = audit_smoke_window_manifest(project_root=project_root, manifest_path=Path(args.manifest_path))

    out_dir = _resolve_project_path(project_root, args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"ml_smoke_window_manifest_audit_{_utc_tag()}.json"
        ...
        json.dumps(
            {
                ...
                "selected_dates": result.get("selected_dates", []),
