...
def main() -> int:
    parser = argparse.ArgumentParser(description="AKFP P10: final short-only cycle.")
    ...
    parser.add_argument("--output-dir", default="reports/akfp/final_short")
    ...
    short_best = best_combos.get("short_only") if isinstance(best_combos, dict) else {}
    combo_filters = []
    if isinstance(short_best, dict):
        combo_filters = [str(x) for x in (short_best.get("combo_filters") or []) if str(x).strip()]
    ...
    report_path = out_dir / f"akfp_short_final_cycle_{symbol}_{timeframe}_{test_date}_{_utc_tag()}.json"
    report_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    package_path = out_dir / "SHORT_FINAL_PACKAGE.json"
