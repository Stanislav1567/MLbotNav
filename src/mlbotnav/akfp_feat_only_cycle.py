...
def _side_run(
    *,
    ...
    cmd = [
        ...
        "--unlock-reason",
        f"AKFP P5 FEAT_ONLY {contour}",
    ]
    run = _run(cmd, project_root)
    return {"contour_id": contour, "adaptive_run": run}


def main() -> int:
    parser = argparse.ArgumentParser(description="AKFP P5: FEAT_ONLY cycle (long/short separately).")
    parser.add_argument("--policy", default="configs/akfp_policy.yaml")
    parser.add_argument("--output-dir", default="reports/akfp/feat_only")
    ...
    out = out_dir / f"akfp_feat_only_cycle_{symbol}_{timeframe}_{test_date}_{_utc_tag()}.json"
