# TZ_OPTUNA_REVISED_BOUNDED_PACKAGE_V44_2026-05-30_RU

Date: 2026-05-30
Scope: Optuna/APTuna only (ML runtime untouched)

## 1. Basis
1. V43 sequence completed with `NO_GO` at `V43-S4`.
2. Observed pattern after V43:
1. long mode remained non-viable (`search_failed` persistence),
2. short mode improved tails in S3 but still below acceptance floors.
...
## 3. Bounded Package V44 (strict)
1. V44-S1: long tradefulness unlock continuation.
2. V44-S2: short entry re-anchor continuation.
3. V44-S3: short risk/move micro-correction.
4. V44-S4: cross-mode confirmation + micro-gate.
...
## 6. DoD for V44 package task
1. V44 package doc created and linked from priorities.
