...
Научить ML выбирать хорошие входы из кандидатов Stas1/Stas2, используя Stas2 и Stas4 контекст, но не ломая старую логику и не превращая стратегии в жесткие фильтры.
...
Stas3 в этот этап не входит. Stas3 позже отвечает за выход, TP и post-entry review.
...
## 10. Этап 7: первый controlled ML

Только после audit:

1. baseline model без yellow `X`;
2. train/validation по дням;
...
| STAS5 ML Ranker | Считает `ML_KEEP_SCORE` | Нет, только score |
| Permission Controller | Превращает score в `ENTER/SKIP/UNSURE` | Да |
