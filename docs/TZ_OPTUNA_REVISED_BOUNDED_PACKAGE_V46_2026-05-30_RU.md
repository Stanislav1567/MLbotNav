# TZ_OPTUNA_REVISED_BOUNDED_PACKAGE_V46_2026-05-30_RU

Date: 2026-05-30
Scope: Optuna/APTuna only (ML runtime untouched)

## 1. Basis
1. V45 sequence completed with `NO_GO` at `V45-S4`.
2. Observed pattern after V45:
1. long mode still non-viable (`search_failed` persistence under current gating shape),
2. short mode tradefulness preserved but acceptance floors still unmet after S2/S3 corrections.
...
## 3. Bounded Package V46 (strict)
1. V46-S1: long tradefulness unlock continuation.
2. V46-S2: short entry re-anchor continuation.
3. V46-S3: short risk/move micro-correction.
4. V46-S4: cross-mode confirmation + micro-gate.
...
## 6. DoD for V46 package task
1. V46 package doc created and linked from priorities.
