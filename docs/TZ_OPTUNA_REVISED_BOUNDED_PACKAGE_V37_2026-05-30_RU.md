# TZ_OPTUNA_REVISED_BOUNDED_PACKAGE_V37_2026-05-30_RU

Date: 2026-05-30
Scope: Optuna/APTuna only (ML runtime untouched)

## 1. Basis
1. V36 sequence completed with `NO_GO` at `V36-S4`.
2. Observed pattern after V36:
...
2. Continue short tail stabilization while preserving tradefulness.
3. Preserve strict one-change-per-step reversibility.

## 3. Bounded Package V37 (strict)
1. V37-S1: long tradefulness unlock continuation
1. single change in `PLongGrid`,
2. keep risk/move settings unchanged.
2. V37-S2: short entry re-anchor continuation
1. single change in `PShortGrid`,
2. preserve V36-S3 risk profile.
3. V37-S3: short risk/move micro-correction
1. single change in `StopLossPct` or `MinExpectedMoveGrid`,
2. preserve V37-S2 entry profile.
4. V37-S4: cross-mode confirmation + micro-gate
...
## 6. DoD for V37 package task
1. V37 package doc created and linked from priorities.
