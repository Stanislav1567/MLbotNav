# TZ_OPTUNA_REVISED_BOUNDED_PACKAGE_V35_2026-05-30_RU

Date: 2026-05-30
Scope: Optuna/APTuna only (ML runtime untouched)

## 1. Basis
1. V34 sequence completed with `NO_GO` at `V34-S4`.
2. Observed pattern after V34:
1. long mode remained in full no-trade collapse,
2. short mode stayed tradeful but still below acceptance quality floors.

## 2. V35 objective
1. Recover long tradefulness from persistent no-trade.
2. Keep short tradefulness and continue tail stabilization.
3. Preserve strict one-change-per-step reversibility.

## 3. Bounded Package V35 (strict)
1. V35-S1: long tradefulness unlock continuation
1. single change in `PLongGrid`,
2. keep risk/move settings unchanged.
2. V35-S2: short entry re-anchor continuation
1. single change in `PShortGrid`,
2. preserve V34-S3 risk profile.
3. V35-S3: short risk/move micro-correction
1. single change in `StopLossPct` or `MinExpectedMoveGrid`,
2. preserve V35-S2 entry profile.
4. V35-S4: cross-mode confirmation + micro-gate
...
## 6. DoD for V35 package task
1. V35 package doc created and linked from priorities.
