# TZ_OPTUNA_REVISED_BOUNDED_PACKAGE_V42_2026-05-30_RU

Date: 2026-05-30
Scope: Optuna/APTuna only (ML runtime untouched)

## 1. Basis
1. V41 sequence completed with `NO_GO` at `V41-S4`.
2. Observed pattern after V41:
1. long mode remained non-viable (`search_failed` persistence),
2. short mode improved dispersion in S3 but still missed acceptance floors.
...
## 3. Bounded Package V42 (strict)
1. V42-S1: long tradefulness unlock continuation.
2. V42-S2: short entry re-anchor continuation.
3. V42-S3: short risk/move micro-correction.
4. V42-S4: cross-mode confirmation + micro-gate.
...
## 6. DoD for V42 package task
1. V42 package doc created and linked from priorities.
