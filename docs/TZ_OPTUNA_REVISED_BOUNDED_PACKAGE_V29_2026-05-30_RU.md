# TZ_OPTUNA_REVISED_BOUNDED_PACKAGE_V29_2026-05-30_RU

Date: 2026-05-30
Scope: Optuna/APTuna only (ML runtime untouched)

## 1. Basis
1. V28 sequence completed with `NO_GO` at `V28-S4`.
2. Observed pattern after V28:
1. long mode remains in full no-trade collapse,
2. short mode tradefulness improves intermittently but quality floors stay far below acceptance.

## 2. V29 objective
1. Recover long tradefulness from no-trade collapse.
2. Stabilize short branches and shrink downside tails.
3. Preserve strict one-change-per-step reversibility.

## 3. Bounded Package V29 (strict)
1. V29-S1: long tradefulness unlock continuation
1. single change in `PLongGrid`,
2. keep risk/move settings unchanged.
2. V29-S2: short entry re-anchor continuation
1. single change in `PShortGrid`,
2. preserve V28-S3 risk profile.
3. V29-S3: short risk/move micro-correction
1. single change in `StopLossPct` or `MinExpectedMoveGrid`,
2. preserve V29-S2 entry profile.
4. V29-S4: cross-mode confirmation + micro-gate
...
## 6. DoD for V29 package task
1. V29 package doc created and linked from priorities.
